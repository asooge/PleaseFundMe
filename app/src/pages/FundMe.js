...
const FundMe = ({ drizzle, drizzleState }) => {
  ...
  const updateFunder = () => {
    ...
    const stackId = drizzle.contracts.PleaseFundMe.methods.updateFunder.cacheSend(
      'updated funder title',
      777,
      { gas: 104320 },
    );
  ...
  const readFunderCount = async () => {
    ...
    const count = await drizzle.contracts.PleaseFundMe.methods
      .fundersCount()
      .call();
    console.log({ count });
  };
