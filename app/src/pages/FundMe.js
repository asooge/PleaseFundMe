...
const FundMe = ({ drizzle, drizzleState }) => {
  ...
  const createFunder = async () => {
    console.log('click')
    console.log(account)
    const stackId = await drizzle.contracts.PleaseFundMe.methods.createFunder.cacheSend('test', 400, { gas: 1043200 })
  ...
  const getFunder = async () => {
    ...
    const funderHash = drizzle.contracts.PleaseFundMe.methods.funders.cacheCall(0)
    console.log({ funderHash })

    // alternatively you can use Web3 to call the function and receive the data
    const funder = await drizzle.contracts.PleaseFundMe.methods.funders(0).call();
    const {
      amountRaised,
      fundBalance,
      fundTarget,
